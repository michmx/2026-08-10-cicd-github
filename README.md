...
[![jupyter-book](https://github.com/hsf-training/hsf-training-cicd-github/actions/workflows/book.yml/badge.svg)](https://github.com/hsf-training/hsf-training-cicd-github/actions/workflows/book.yml)
...
The lesson is built with [Jupyter Book](https://jupyterbook.org/). To render it locally:

```bash
python -m venv .venv
source .venv/bin/activate
pip install -r requirements.txt
jupyter-book build cicd_github/
```

The rendered HTML lands in `cicd_github/_build/html/`. Preview it with

```bash
python -m http.server -d cicd_github/_build/html 8000
```

CI builds the book in strict mode (`jupyter-book build -W --keep-going cicd_github/`) and deploys it to GitHub Pages on pushes to `gh-pages`.
...
We'd like to ask you to familiarize yourself with our [Contribution Guide](CONTRIBUTING.md). Lesson pages are
[MyST Markdown](https://myst-parser.readthedocs.io/) files in `cicd_github/`; new pages must be registered in
`cicd_github/_toc.yml`.
